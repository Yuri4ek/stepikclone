...
import { formatPercent, hoursSince, plural, useAsync } from '@/shared/lib'
import { ButtonLink, Card, EmptyState, ErrorBox, Loader, PageHeader } from '@/shared/ui'

async function load() {
  const [queue, critical, warning] = await Promise.all([
    submissionApi.queue({ limit: 100 }),
    lagApi.students({ level: 'critical', limit: 5 }),
    lagApi.students({ level: 'warning', limit: 5 }),
  ])
