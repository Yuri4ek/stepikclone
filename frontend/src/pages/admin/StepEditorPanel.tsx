...
import { api, type AdminStep, type LearningStep, type StepContent } from '../../api'
import { checkLabels } from '../../steps/registry'
import type { StepTypeDef } from '../../steps/types'
import { StepTypeBadge } from '../../components/StepTypeBadge'
import { Button, Card, ErrorBox, Field, Input, Notice, Segmented } from '../../components/ui'
...
export function StepEditorPanel({ type, step, lessonId, nextPosition, onSaved, onDeleted, onCancel }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = step ? await api.admin.updateStep(step.id, data) : await api.admin.addStep(lessonId, { ...data, kind: type.kind, position: nextPosition })
      setSaved(true)
  ...
  const remove = async () => {
    ...
    setBusy('delete')
    try {
      await api.admin.deleteStep(step.id)
      onDeleted()
    } catch (e) {
      setError(e as Error)
      setBusy(null)
