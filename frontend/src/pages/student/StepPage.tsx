...
import { api, ApiError, type Answers, type LearningStep, type SubmitResult } from '../../api'
import { formatScore } from '../../lib/format'
import { logSubmission } from '../../lib/submissionLog'
import { useUser } from '../../auth/AuthContext'
import { flattenOutline, sortOutline, type FlatStep } from '../../lib/outline'
import { useAsync } from '../../lib/useAsync'
import { checkLabels, resolveStepType } from '../../steps/registry'
import type { StepTypeDef } from '../../steps/types'
import { StepTypeBadge } from '../../components/StepTypeBadge'
import { Button, Card, ErrorBox, Loader, Notice, ProgressBar, ScorePill, StatusBadge, cx, stepStatusMeta } from '../../components/ui'
...
export function StepPage() {
  ...
  const user = useUser()
  const outline = useAsync(() => api.catalog.outline(courseId).then(sortOutline), [courseId])
  const step = useAsync(() => api.learning.step(stepId), [stepId])
  const progress = useAsync(() => api.catalog.next(courseId), [courseId])
  ...
  const submit = (answers: Answers) =>
    act(async () => {
      const data = await api.learning.submit(stepId, answers)
  ...
  const complete = () =>
    act(async () => {
      await api.learning.complete(stepId)
