...
def unlock_next(db: Session, user_id: uuid.UUID, course_id: uuid.UUID) -> None:
    ...
    for step in steps:
        ...
        if sp.status == StepProgressStatus.locked:
            ...
            db.add(sp)
            break
        if sp.status not in (StepProgressStatus.passed,):
            break
...
def recalculate(db: Session, enrollment: Enrollment) -> CourseProgress:
    ...
    for step in steps:
        ...
        if sp is None:
            continue
        if current_step_id is None and sp.status not in (StepProgressStatus.passed,):
                ...
                breakdown.append(
                    {
                        ...
                        "source": source_for_step(db, enrollment.user_id, step.id, step.kind.value),
                    }
                )
